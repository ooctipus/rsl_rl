...
class TrajectoryBuffer:
    ...
    def __post_init__(self) -> None:
        ...
        # vectorized-sampler cache (built lazily, invalidated on extend)
        self._flat = None          # {key: [total_frames, dim]} contiguous concat of all episodes
        self._ep_starts = None     # [num_eps] long: start offset of each episode in the flat tensor
        self._ep_lengths = None    # [num_eps] long: per-episode frame count
        self._flat_seq_length = None  # seq_length the cached _ep_valid was built for
        self._ep_valid = None      # [num_eps] long: valid start range = length - seq_length - offset
    ...
    def extend(self, data: List[dict]) -> None:
        ...
        # episodes changed -> drop the flat cache so the next sample() rebuilds it
        self._flat = None

    def _build_flat_cache(self) -> None:
        """Concatenate all episodes into one contiguous tensor per consumed key, plus an
        ``_ep_starts`` offset table, so ``sample`` can advanced-index instead of looping.

        Built lazily on first ``sample`` (after all ``extend`` calls). Equivalent to the
        per-slice Python loop: episode ``e`` occupies rows ``[_ep_starts[e], _ep_starts[e]+len_e)``
        of every flat key tensor, so slicing ``flat[start:start+seq_length]`` reproduces
        ``storage[e][k][t:t+seq_length]`` exactly (same dtype, device, and contiguous frames).
        """
        keys = list(dict.fromkeys(list(self.output_key_t) + list(self.output_key_tp1)))
        n_eps = len(self)
        lengths = [self.storage[e][self.output_key_t[0]].shape[0] for e in range(n_eps)]
        self._ep_lengths = torch.tensor(lengths, device=self.device, dtype=torch.long)
        starts = torch.zeros(n_eps, device=self.device, dtype=torch.long)
        if n_eps > 1:
            starts[1:] = torch.cumsum(self._ep_lengths[:-1], dim=0)
        self._ep_starts = starts
        self._flat = {k: torch.cat([self.storage[e][k] for e in range(n_eps)], dim=0) for k in keys}
        self._flat_seq_length = None  # force _ep_valid rebuild against current seq_length
    ...
    def sample(self, batch_size: int = 1) -> Dict[str, torch.Tensor]:
        ...
        offset = 1 if len(self.output_key_tp1) > 0 else 0

        if self._flat is None:
            self._build_flat_cache()
        if self._flat_seq_length != self.seq_length:
            # valid start range per episode == range of the original randint(0, length - seq - offset)
            self._ep_valid = self._ep_lengths - self.seq_length - offset
            self._flat_seq_length = self.seq_length

        # --- episode selection: identical to the original (multinomial over priorities) ---
        self.ep_ind = torch.multinomial(self.priorities, num_slices, replacement=True)

        # --- start-frame selection: vectorized uniform over [0, valid_i) per sampled episode ---
        # Original drew torch.randint(0, length_i - seq - offset) independently per slice; this is
        # the same distribution (uniform on {0,...,valid_i-1}) computed in one kernel.
        valid = self._ep_valid[self.ep_ind]                                  # [num_slices]
        time_idx = (torch.rand(num_slices, device=self.device) * valid.to(torch.float32)).to(torch.long)
        time_idx = torch.minimum(time_idx, valid - 1).clamp_(min=0)          # guard fp edge (rand==1-eps)

        # --- flat advanced index: [num_slices, seq_length] row-major == per-slice cat order ---
        base_t = self._ep_starts[self.ep_ind] + time_idx                     # [num_slices]
        seq_ar = torch.arange(self.seq_length, device=self.device)           # [seq_length]
        idx_t = (base_t.unsqueeze(1) + seq_ar.unsqueeze(0)).reshape(-1)      # [num_slices*seq_length]

        output = {}
        for k in self.output_key_t:
            output[k] = self._flat[k].index_select(0, idx_t)
        if offset:
            idx_tp1 = (base_t.unsqueeze(1) + offset + seq_ar.unsqueeze(0)).reshape(-1)
            output["next"] = {k: self._flat[k].index_select(0, idx_tp1) for k in self.output_key_tp1}
        return output
